    ...
    for (const [key, value] of formData.entries()) {
        ...
        if (typeof parsedValue === "string") {
            ...
            if (!isNaN(parsedValue) && parsedValue !== "") {
                if (String(Number(parsedValue)) === parsedValue || String(parseInt(parsedValue, 10)) === parsedValue) {
...
function highlightJson(json) {
    ...
    formattedJson = formattedJson.replace(
        ...
        '<span class="json-number">$1$3</span>'
    );
 ...
 * @returns {HTMLStyleElement | null} The created style element or null if already present.
 */
function createStyles() {
    ...
    style.textContent = `
        ...
        .html-form-json-container h1 {
            font-size: 1.5rem;
            color: var(--primary-color);
            text-align: center;
            margin-bottom: 25px;
        }
        .html-form-json-container h2 {
...
function renderFormJson(elementId) {
    ...
    // 1. Add Styles to the document head
    createStyles();
    ...
    targetElement.innerHTML = `
        <div class="html-form-json-container" id="${elementId}-container">
            ...
            <textarea
                id="${inputId}"
                placeholder="Example: &lt;form action='/submit' method='post'&gt;..."
            ></textarea>
    ...
    const formatButtonListener = () => {
        if (!htmlInput || !jsonOutput || !copyButton) {
            return;
        }
    ...
    const copyButtonListener = () => {
        ...
        if (!navigator.clipboard || typeof navigator.clipboard.writeText !== "function") {
            // Simple fallback: prompt-based copy
            window.prompt("Copy the JSON below:", textToCopy);
            return;
        }
    ...
    if (formatButton) {
        formatButton.addEventListener("click", formatButtonListener);
    }
    if (copyButton) {
    ...
    return () => {
        if (formatButton) {
            formatButton.removeEventListener("click", formatButtonListener);
        }
        if (copyButton) {
